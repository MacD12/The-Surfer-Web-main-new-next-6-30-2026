...
type Room = { key: RoomKey; title: string; desc: string; alt: string };
...
const ROOM_META: Record<RoomKey, { capacity: string }> = {
  dormitory: { capacity: 'Up to 5 guests' },
  singleRoom: { capacity: '1 guest' },
  doubleRoom: { capacity: '2 guests' },
  tripleRoom: { capacity: '3 guests' },
};

const ComfortableStays = () => {
  const { t } = useTranslation();
  const rooms = t('comfortableStays.rooms', { returnObjects: true }) as Room[];
